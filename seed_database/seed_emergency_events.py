...
with open("populate_emergency_events.sql", "w") as f:
    ...
    for i in range(1, 101, 100):
        ...
        for j in range(i, i + 100):
            ...
            values.append(f"('{id}', '{description}', '{location}', '{latitude}', '{longitude}', '{severity}', '{status}', '{reported_at}', '{updated_at}', '{type}')")
        # Write a batch of 1000 records to the file
        f.write(f"INSERT INTO \"EmergencyEvents\" (\"Id\", \"Description\", \"Location\", \"Latitude\", \"Longitude\", \"Severity\", \"Status\", \"ReportedAt\", \"UpdatedAt\", \"Type\") VALUES {', '.join(values)};\n")